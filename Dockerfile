ARG BASE_IMAGE=lustefaniak/docker-graalvm:alpine-19.1.1-2
FROM ${BASE_IMAGE}
...
RUN mkdir -p src/main/scala project && echo sbt.version=${SBT_VERSION} > project/build.properties && echo "object A" > src/main/scala/A.scala && sbt 'set scalaVersion:="'${SCALA_VERSION}'"' compile && rm -r -f src target project
